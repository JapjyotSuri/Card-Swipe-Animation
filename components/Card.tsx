...
const Card = ({
    ...
    .onUpdate((e) => {
      ...
      if (currentIndex === index) {
        ...
        animatedValue.value = interpolate(
          ...
          [0, width],
          [index, index + 1]
        ); //here we dont write [0,1] in output but give index to preserve the stack effect
    ...
    .onEnd((e) => {
      if (index === currentIndex) {
        if (Math.abs(e.translationX) > 150 || Math.abs(e.velocityX) > 1000) {
          //meaning the abs value of translationX is greater than 150 then we need to delete it
  ...
  const animatedStyle = useAnimatedStyle(() => {
    //A combination of translateY and scale is causing the cards to appear to move in the forward direction in the z axis when a card is removed
    ...
    const translateY = interpolate(
      ...
      [index - 1, index],
      [-30, 0]
    ); //index keep on changing so first the value index-1 means that the card we are currently writing th ecode is the card tht is below the the currentIndex meaning currentIndex=index-1 where index is the index of the card whose code is being written
    ...
    const opacity = interpolate(
      ...
      [index, index + 1],
      [0, 1]
    ); //animatedValue.value + 3 ensures cards that are 3 or more layers deep are partially transparent.
    return {
      opacity: index < 3 + currentIndex ? 1 : opacity,
      transform: [
        {
          translateX: translateX.value, //moving the card horizontally
        },
        {
          scale: currentIndex === index ? 1 : scale, //last index is going to be the smallest and 0th index is going to be the biggest
        },
        {
          translateY: currentIndex === index ? 0 : translateY, //responsible to show cards in the stack effect
        },
        {
          rotateZ:
            index === currentIndex ? `${direction.value * rotateZ}deg` : "0deg", //responsible to make the card appear to rotate when moved horizontally
        },
      ],
    };
  });
